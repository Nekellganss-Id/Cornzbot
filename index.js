...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('CORNZ-BOT', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'NEKELLGANSS', 'ubuntu', '3.0' ]
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `CORNZ-BOT`, img,but = [{buttonId:`donasi`, 

               buttonText:{displayText: 'WELCOME IDOLL'},type:1}], options = {contextInfo: {mentionedJid: [num, htod]}})
			...
			try {
				...
				sendButImage(anu.jid, teks, `CORNZ-BOT`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
